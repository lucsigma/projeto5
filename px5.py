...
conn = sqlite3.connect("px5.db", check_same_thread=False)
cursor = conn.cursor()
...
produtos_lista = {
    ...
    "j9": "batata branca","l10": "alho","m11": "manutenção","n12": "gengibre","o13": "ovos","p14": "maçã","q15": "melancia"
}
...
tipo = st.radio("Tipo de embalagem:", ["Caixa", "Saco", "Unidade"])
...
# 📥 Download do estoque em CSV e TSV
if not df_produtos.empty:
    # CSV
    ...
    # TSV
    tsv_data = df_produtos.to_csv(sep="\t", index=False)
    st.download_button(
        label="📥 Baixar Estoque em TSV",
        data=tsv_data.encode("utf-8"),
        file_name="estoque_atual.tsv",
